//
// Created by Nathan Welch on 4/28/2017.
//
...
void max_flow(int graph[V][V], int s, int t) {
  ...
  ///when one of the "edges" in here get to zero it means no more flow can be passed through it
...
bool path_finder(int residual[V][V], int s, int t, int path[])
{
  ...
  while ( !q.empty() )
  {
    int u = q.front(); ///looks at all the "edges" that could be hooked up to the "node" at the top
    q.pop();  ///takes the first thing out of the queue

    for (int v = 0; v < V; v++) ///happens until there are no "nodes" left then it goes back up to the while and pops another one
    {
      ///and if they haven't been visited and have residual left then visit it
